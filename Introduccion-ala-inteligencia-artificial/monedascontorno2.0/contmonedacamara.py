...
def alineamiento(imagen,ancho,alto):
    ...
    _,umbral=cv2.threshold(grises, 150,255, cv2.THRESH_BINARY)#ahora transformamos a B&W la imagen
    cv2.imshow("Umbral", umbral)#Con la funcion .imshow mostramos la imagen
    ...
    for c in contorno:  #le decimos que nuestro interador= i recorra en contornos para sacra informacion
...
while True:
    ...
    if imagen_A6 is not None:
        ...
        for c_2 in contorno2:
            ...
            #si estamos en un momneto estatico == 0
            if(Momentos["m00"]==0):
                #pues este momento deve estar en 1.0 si esta estatico
                Momentos["m00"]=1.0
                #le decimos que la cordenada x ["m10"] se desplasca al momento estatico ["m00"]
                # / va a estar dividido por el momento estatico
            x=int(Momentos["m10"]/Momentos["m00"])
            #le decimos que la cordenada y ["m01"] se desplasca al momento estatico ["m00"], 
            # / va a estar dividido por el momento estatico
            y=int(Momentos["m01"]/Momentos["m00"])

#ahora vamos amedir las monedas para saver que moneda es, si mide 9300 = es una moneda de 20 centimos
        #le decimos que si area es menor a 9300 y mayor a 8000
            if area<9300 and area>8000:
                #le damos el tipo de letra que sera un HERSHEY_SIMPLEX, con la funcion = v2.FONT_
                font=cv2.FONT_HERSHEY_SIMPLEX
                #entonces si area es menor a 9300 y mayor a 8000, se trata de una moneda de "S/. 0.20"
             #con la funcion cv2.putText, hacemos que aparesca el texto, que sea que es una moneda de 20 centimos
             #nos piden los siguientes parametros:
             # (la imagen, "el texto a mostrar",las cordenadas a ubicar"x , y",
             # el tipo de letra=font, la escala de letra=0.75, el color que nos muestre=0,255,0) y el grosor= 2 )
            ...
            if area<7800 and area>6500:
                ...
                suma2=suma2+0.1
        total=suma1+suma2
        #le decimos que nos imprima el total round(total,2) = pero solo con 2 desimales
        print("Sumatoria total en Centimos:",round(total,2))
        cv2.imshow("Imagen A6", imagen_A6)
        cv2.imshow("camara", camara)
        #le decimos que si apretamos s, se cierre la ventana o la camara
